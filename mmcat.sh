...
EXTRA_OPTIONS='-s 1920x1080 -vcodec h264'
PIP='-filter_complex "[0:1]scale=iw/3:ih/3 [pip]; [0:0][pip] overlay=main_w-overlay_w-30:main_h-overlay_h-60"'
...
bash -c "ffmpeg -y -i $first $PIP -an -f yuv4mpegpipe -vcodec rawvideo $TMP/mcs_v1 </dev/null" &
#2>/dev/null
...
if [ "$inputs" -ne "" ]; then
	...
	do
		...
		{ bash -c "ffmpeg -y -i $f $PIP -an -f yuv4mpegpipe -vcodec rawvideo - 2>/dev/null </dev/null | tail -n +2 > $TMP/mcs_v$i" ; } &
